...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (float x = std::floor(bbox[0].x()); x <= std::ceil(bbox[1].x()); x++)
    {
        for (float y = std::floor(bbox[0].y()); y <=std::ceil( bbox[1].y()); y++)//floor 和ceil是向两侧取整
        {
            if (insideTriangle(x, y, t.v))
            {
                //计算重心坐标
                float alpha, beta, gamma;
                std::tie(alpha, beta, gamma) = computeBarycentric2D(x+0.5, y+0.5, t.v);//解包获得元组的元素值，需要先声明变量，见上一行
                ...
                if (z_interpolated < depth_buf[get_index(x, y)])
                {
                    ...
                    //法线坐标
                    auto interpolated_normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], 1.0f).normalized();//记得要单位化，传入法线参数时需要标准化
                    //纹理坐标
                    ...
                    depth_buf[get_index(x, y)] = z_interpolated;//更新深度数组
    ...
    //实现自动插值颜色 法线 纹理颜色等的插值
    // auto interpolated_color
    // auto interpolated_normal
    // auto interpolated_texcoords
    // auto interpolated_shadingcoords//实现位移贴图时考虑的
